import { EvaluatorError } from '../../src/services/evaluator/evaluatorErrors';
...
describe("Evaluator service tests", () => {
    ...
    describe("Forbidden characters in input", () => {
        ...
        it("Should throw correct error message", () => {
            expect(() => (evaluator.eval(input))).toThrow(/^Expected .*$/);
        });
    ...
    describe("Expression shouldn't be followed by an expression", () => {
        ...
        it("Should throw correct error message", () => {
            expect(() => (evaluator.eval(input))).toThrow(/^Expected character.*$/);
        });
    ...
    describe("Operator should be followed by expression", () => {
        ...
        it("Should throw correct error message", () => {
            expect(() => (evaluator.eval(input))).toThrow(/^Expected '\(' or number, but.*$/);
        });
    ...
    describe.each([
        ...
        ["  ", 0, 2],
        [" ", 0, 1]
    ])("Input should start with expression '%s'", (input: string, expectedStart: number, expectedEnd: number) => {
        it("Should throw correct error type", () => {
            expect(() => (evaluator.eval(input))).toThrow(EvaluatorError);
        });
